...
## 5.7. Python을 쓰지 않는 이유라도 있나요?

디시인사이드 등에서 타사이트 관련 질문에 답변을 해드리면 `꼬우면 파이썬 쓰던가`라는 댓글이 많았습니다. 저는 파이썬에 익숙하지 않으며, 파이썬과 C#.NET을 연동하는 방법도 알지 못합니다. 따라서 파이썬을 못쓰고 개발한 것입니다.
