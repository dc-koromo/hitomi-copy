...
<span style="display:block;text-align:center">![hitomi main](Docs/Images/18.png)</span>

작가/그룹창 오른쪽 메뉴에서 `자세히 보기`를 누르면 위와 같은 창이 나나탑니다. 이 창에선 해당 작가/그룹의 모든 시리즈 목록, 캐릭터 목록을 보여주며, 해당 작가/그룹과 가장 유사한 작가를 하단에 보여줍니다.
...
히토미 카피엔 개발자 및 마이너 유저를 위한 여러가지 고급 기능들이 포함되어 있습니다. 이런 기능이 존재하는 가장 큰 이유는 개발자가 새로운 기능을 테스트하기 위함이며, 기타 다운로드 오류를 캡쳐하거나, 아직 구현되지 않은 기능이나 사라진 기능을 사용자에게 제공하기 위함입니다. 고급 기능을 사용하려면 `고급설정->UsingLog`를 활성화 해야합니다.

## 2.1 명령어 목록

`Log`에서 `help` 명령어를 통해 명령어의 목록과 간단한 설명을 볼 수 있습니다.

| 명령어 | 설명 |
|---|---|
|enum, enumi, enumx, ei|프로그램 개체의 변수 목록을 나열합니다.|
|get, set|특정 변수의 상태를 가져오거나 설정하는 명령입니다.|
|ra|작가 추천 기능을 제어할 수 있는 명령입니다.|
|install|실험용 도구를 다운로드하기 위한 명령입니다.|
|exh|Ex-Hentai 기능을 실험하기 위한 명령입니다.|
|tk|영문태그의 번역을 보여줍니다.|
|rt|연광태그 분석기를 실행합니다.|
|gv|그래프뷰어를 실행합니다.|
|load|특정 폴더에 있는 모든 메타데이터 json파일을 불러옵니다.|
|merge|JSon 파일 머지툴을 실행합니다.|
|tagr|태그데이터를 재빌드합니다.|
|rtt|연관태그 기반 분석을 위한 명령입니다.|
|ct, gbt|403 Forbidden 데이터를 작성하기 위한 도구입니다.|
|latest|가장 최근의 메타데이터 Id를 가져옵니다.|
|mm|마루마루 기능을 실험하기 위한 명령입니다.|
|record|기록 창을 실행합니다.|
|finder|Finder를 실행합니다.|
|series|모든 시리즈를 switch case로 나열합니다.|
|manual|메뉴얼을 실행합니다.|
|pv|패키지 뷰어를 실행합니다.|
|pm|패키지 메이커를 실행합니다.|
|fs|파일 시스템 매니져를 실행합니다.|

---

## 2.2. enum, get/set 명령어

```
enum, enumi, enumx, ei, get, set명령이 이 시스템을 위한 명령들 입니다.
enum : 클래스에 포함된 모든 public 변수를 열거합니다.
enumi : 클래스에 포함된 모든 public, private 변수를 열거합니다.
enumx : 클래스에 포함된 static 변수를 제외한 모든 변수를 열거합니다. 열거엔 부모 클래스도 포함됩니다.
ei : 인스턴스를 나열하거나 인스턴스에 포함된 변수를 나열합니다.

get/set 명령어는 특정 인스턴스 변수에 값을 가져오거나 설정합니다.
get frmMain latest_load_count
set frmMain latest_load_count 100
처럼 사용하면 됩니다.
```

이 명령을 이해하기 위해선 Hitomi Copy의 소스코드를 이해할 필요가 있습니다. 먼저 `enum`만 입력하면 세 가지 목록이 나올 겁니다. `frmMain`, `MetroFlatDropShadow`, `Log` 이는 프로그램의 주 핸들을 나타냅니다. `frmMain`은 메인창, `MetroFlatDropShadow`는 메인창 뒷부분의 그림자, `Log`는 `Log`창입니다. `enum frmMain`을 입력하면 `frmMain`에 포함된 변수목록이 보입니다. 이 변수 목록은 public 변수만 나타납니다. 여기서 하위 클래스를 보려면 `enum frmMain download_queue`와 같이 연속으로 입력하면 됩니다. `enum` 명령은 public 메서드만, `enumi`는 private메서드도 같이, `enumx`는 모든 부모클래스 인터페이스의 변수를 전부다 보여줍니다. `ei`는 Instance를 위한 명령어입니다. 주 핸들에는 Instance가 포함되지 않기 때문에 Instance를 접근하려면 `ei` 명령이 필요합니다. 사용가능한 Instance 목록은 `ei`를 입력하고 확인하세요.

---

## 2.3. ra 명령어

히토미 카피는 `ra system`을 통해 강력한 작가 추천 기능을 제공합니다.

<span style="display:block;text-align:center">![hitomi main](Docs/hhh.png)</span>
...
다음은 ra 명령어를 사용하는 방법입니다.

```
ra on ; 커스텀 분석 시작 (ra off 도 구현됨)
ra mion ; 커스텀 태그에 추가된 것들이 모두 작품에 있을 때만 작가 추천항목에 넣음 (ra mioff 도 구현됨)
ra update ; 작가 추천 목록 업데이트

이제 원하는 태그나 작가를 추가합니다.
ra + female:big_breasts 1 ; 태그 추가 (마지막 숫자는 점수)
ra +a hisasi ; 작가 추가 (작가가 가진 모든 태그를 추가함)

다음은 목록을 생성하는데 유용한 명령어 입니다.
ra add sekiya_asami ; 수동으로 작가 추가
ra list ; 모든 커스텀 태그 열거
ra rank ; 점수 내림차순으로 작가 추천 목록 열거 (ra update가 먼저 수행되어야 합니다.)
get hitomi_analysis ; 점수 오름차순으로 작가 추천 목록 열거 (ra update가 먼저 수행되어야 합니다.)=
ra clear ; 커스텀 태그 모두 삭제
ra addbk ; 북마크에 있는 모든 작가 추가
ra +abk ; 북마크에 있는 모든 작가의 태그 추가
```
...
```
ct는 Related Galleries를 분석하기 위한 도구입니다. gbt가 개발되기전 히든갤러리를 찾기 위해 구현되었습니다.
gbt는 히든갤러리를 찾기 위한 전수조사 도구입니다.
latest명령을 이용해 최신 갤러리 번호를 복사하고 gbt를 사용하세요.
```
...
히든데이터나 메타데이터, 로그 데이터를 합치기 위한 도구 입니다. 대상에 파일이 있는 경로, 또는 파일 이름을 입력하고 데이터를 불러온 뒤 저장하면 됩니다.

---

## 2.6. rt 툴, rtt 기능

연관 태그 분석 기능입니다. 연관 태그 분석은 모든 메타데이터를 이용해 분석합니다. 서로 다른 두 태그에 연관도를 분석하기 위해 두 태그 중 하나라도 포함된 모든 메타데이터를 분석하고 두 태그가 모두 포함된 메타데이터의 비율을 계산합니다.

<span style="display:block;text-align:center">![hitomi main](Docs/Images/21.png)</span>
...
<span style="display:block;text-align:center">![hitomi main](Docs/Images/23.png)</span>

연관도 순으로 정렬된 목록을 3시 부분부터 시계방향으로 보여줍니다.

rtt는 현재 하나의 명령어만 지원합니다. `rtt isotest`는 위 연관 태그 분석 결과를 바탕으로 고립 태그 그룹을 분석합니다. 고립 태그 분석은 연관성이 있는 모든 태그를 그룹짓습니다.

---

## 2.8. File System Manager

파일 시스템 매니져는 히토미 다운로드 작품을 위한 강력한 파일 정리 도구입니다. 파일 시스템을 매니져를 사용하기 위해선 파일/폴더 이름에 다운로드 작품을 식별할 수 있는 ID가 반드시 포함되어야 합니다.

<span style="display:block;text-align:center">![hitomi main](Docs/Images/24.png)</span>

먼저 정리하려는 `경로`를 입력합니다. 그 다음 `열기` 버튼을 눌러 모든 파일/폴더를 불러올 때까지 기다립니다. 이 과정동안 프로그램이 잠시 멈출 수 있습니다. 다음으로 특정 파일/폴더의 이름을 필터링할 규칙을 입력합니다. 이 규칙은 Regular Expression로 작성되어야하며, 첫 번째 그룹에 ID가 포함되어야합니다. 기본 규칙들은 다음과 같습니다.


| 규칙 | 설명 |
|---|---|
|^\[(\d+)\]|히토미 카피의 초기 경로 설정의 규칙입니다. [123456] Title 과 같은 형식이 이 규칙에 해당합니다.|
|^\[.*?\((\d+)\).*?\]|Hi Downloader의 초기 경로 설정의 규칙입니다. [(123546) Hinara Emi] Title과 같은 형식이 이 규칙에 해당합니다.|
|\((\d+)\)$|Hitomi Downloader GUI의 초기 경로 설정의 규칙입니다. [Hinara Emi] Title (123456)과 같은 형식이 이 규칙에 해당합니다.|

규칙을 입력하고 `적용` 버튼을 누르면 가용성있는 항목이 나타나게 됩니다. 가용성이 없다는 건 추출된 이름(ID)로부터 작품 데이터를 가져올 수 없음을 의미힙니다. 이 규칙 적용에선 파일이든 폴더든 이름이 매칭된다면 인덱싱됩니다. 또한 파일의 경우 확장자가 생략된 상태로 검사됩니다. 모든 가용성있는 항목을 검사한 뒤 상단의 `작업`탭으로 넘어갑니다.

<span style="display:block;text-align:center">![hitomi main](Docs/Images/25.png)</span>

`작업`에는 `재배치`, `이름 바꾸기`, `뷰어` 도구가 있습니다. `재배치` 도구에는 작가나 그룹의 우선순위가 표시되며, 이동 규칙을 통해 파일을 이동시킬 수 있습니다. `이름 바꾸기`도구는 파일또는 폴더의 이름만들 바꿉니다. `뷰어`는 알파벳 순서대로 100개의 작품목록을 보여줍니다. (이 뷰어 기능은 추후에 완벽히 개발할 계획입니다.)

`이동 시험` 및 `이름 바꾸기 시험`에서 표시되는 오류 내용으로는 `Already exists`와 `Overlapping`이 있습니다. `Already exists`는 바꾸려는 경로에 파일 또는 폴더가 이미 존재한다는 뜻이고, `Overlapping`은 상위 파일 또는 폴더이름과 중복된다는 뜻입니다.

`재배치` 및 `이름 바꾸기`를 실행하기 전 반드시 `시험` 결과를 확인하시고 진행하시기 바랍니다. 잘못된 사용으로 인해 발생한 문제는 되돌릴 수 없으니 참고하세요.
...
```
install driver : 드라이버를 다운로드합니다. 히토미 세마포어를 멀티 쓰레딩에서 멀티 프로세스로 변경할 수 있습니다. 이 기능을 사용하려면 setting.json에서 UsingDriver 설정을 활성화시키세요.
tagr : 현재는 프로그램 시작시, 언어변경시 자동으로 수행하므로 필요없습니다.
```
...
## 3.1. 데이터 다운로드시, 데이터 동기화시 오류가 발생합니다
...
현재는 히토미/익헨/마루마루를 제외한 나머지 사이트를 개발할 계획이 없습니다. 해당 기능을 원한다면, 직접 기능을 추가해 `Pull Request`하시기 바랍니다. 다른 분이 만든 히토미 다운로더인 `Hitomi Downloader GUI` (https://github.com/KurtBestor/Hitomi-Downloader-issues)가 많은 사이트를 지원하니 여러 사이트를 원한다면 이 다운로더를 추천드립니다.

## 4.2. 다른 다운로더와의 호환성에 관하여

2주에 한 번 꼴로 `Hitomi Downloader GUI`에 관한 질문을 받는 것 같습니다. `Hitomi Downloader GUI`의 히토미 특화 버전으로 착각해 다운로드 목록이 호환이 안된다는 질문이 대부분이였습니다. 따라서 신규 유입 유저분들을 위해 `Hitomi Downloader GUI`의 로그를 분석해주는 기능을 추가할 계획이며, `Hitomi Downloader GUI`를 위한 로그를 내보내는 기능 또한 만들 예정입니다.

## 4.3. 강력한 다운로더를 향하여

`Hitomi Copy 4.0` 개발을 시작했습니다. UI는 비슷하나 프로그램 설계상 많은 것들이 달라질 예정입니다. 플러그인 형식으로 다른 사이트 다운로드를 지원할 계획이며, 이 플러그인을 사용자가 직접 제작할 수 있게 간단한 스크립트 언어도 설계할 계획입니다. 또한 이 스크립트를 작성하기 위한 편리한 Html 분석도구도 만들 예정입니다.

---

# 5. 기타 여러가지

## 5.1. 검색결과의 수를 제한하는 이유는 무엇인가요?
...
히토미 카피 유저의 수는 많지 않습니다. 자동업데이트 기반으로 트래픽을 측정하면, 한 버전당 1,000 명 내외입니다. 다만, github 통계 누적을 통해 분석한 총 다운로드 유저의 수는 30,000건이 넘습니다. Clone의 수는 일주일에 3~4건 정도로 적은 편입니다.

## 5.4. 더이상 히든데이터는 업데이트되지 않습니다.

7월 초 히토미 서버 개편이후로 더이상 403 Forbidden 데이터를 서버에 남겨두지 않습니다. 익펀을 이용해 사라진 데이터를 다운로드해주세요.

## 5.5. 모바일 버전으로 만들 계획이 있나요?

없습니다. 모바일 버전의 히토미/이헨/익헨 뷰어 다운로더는 이미 많은 분들에게 익숙한 프로그램들이 있습니다. 새로운 프로그램이 만들어지면 새로운 프로그램에 익숙해지는 시간이나 모든 데이터를 수동으로 옮겨야되는 불편함 등이 있어 사용자가 매우 적을 것으로 생각됩니다. 따라서 모바일 버전은 개발할 계획이 없습니다.

## 5.6. 메모리 사용량이 너무 많은 것같아요.

히토미 카피는 작가 추천, 자동 완성, 빠른 검색을 지원하기 위해 모든 작품 데이터를 메모리에 로드합니다. 따라서 기본적으로 많은 메모리 자원을 소모합니다. 지속적으로 메모리 사용량에 관한 이슈를 해결하는 중이며 메모리 사용량을 최소한으로 줄이도록 노력하겠습니다.

---

히토미 카피는 오픈소스 프로젝트이며 MIT License 기반으로 출처만 남긴다면 누구나 소스코드의 일부 또는 전체를 수정, 배포할 수 있습니다.
